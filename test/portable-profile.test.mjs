...
test('portable declarations retain explicit session delivery, private context and wider consent without native probes',t=>{
 ...
 const unconfigured=invoke(f,'check',request('check',{binding:p.binding,context,action:{kind:'inspect'}}));assert.deepEqual(unconfigured.value.result,{status:'needs-configuration',problems:[{code:'needs-configuration',message:`no messaging root at ${f.base}: run oats aweb setup there or set settings.oats.aweb.root`},{code:'needs-configuration',message:'no team: set settings.oats.aweb.team or keep an active team at the aweb root'}]},'host readiness problems are reported before captured invocation is required');
...
test('binding-less workspace readiness check follows settings roots, team labels and read-only custody',t=>{
 ...
 assert.deepEqual(checked.value.result,{status:'needs-configuration',problems:[{code:'needs-configuration',message:`no messaging root at ${absent}: run oats aweb setup there or set settings.oats.aweb.root`},{code:'needs-configuration',message:'no team: set settings.oats.aweb.team or keep an active team at the aweb root'}],warnings:[]});
 ...
 assert.deepEqual(checked.value.result,{status:'needs-configuration',problems:[{code:'needs-configuration',message:'no team: set settings.oats.aweb.team or keep an active team at the aweb root'}],warnings:[]});
...
test('manifest keeps required messaging and both delivery resource closures, with canonical codec commands',()=>{
 ...
 assert.equal(distribution.compatibility.oats,m.compatibility.oats);for(const value of [m,distribution,tooling])assert.equal(value.version,'1.14.1');
});
